...
impl Into<Program> for CompUnit {
  fn into(self) -> Program {
    ...
    let main = program.new_func(
      FunctionData::new(format!("@{}", self.func_def.ident), Vec::new(), self.func_def.func_type.into()),
    );
    ...
    let value = main_data.dfg_mut().new_value().integer(self.func_def.block.stmt.number);
